...
const uint8_t mCHANNEL = 3;
const uint8_t mMAC = 0x43;
const String mSSID = "0x03-Turing";
...
uint32_t COMPUTE_PERIOD_MS = 4000;
...
void loop() {
  if ((DATA_IN_CNT >= DATA_IN_SIZE) && (millis() - lastComputeTime > COMPUTE_PERIOD_MS)) {
    ...
    Reaction mReaction = Reaction(DATA_IN, DATA_IN_SIZE);

    for (int i = 0; i < DATA_OUT_SIZE / 2; i++) {
      Vec2 ab = mReaction.AB(i);
      DATA_OUT[2 * i + 0] = (uint8_t)(int(ab.x * 255.0) & 0xFF);
      DATA_OUT[2 * i + 1] = (uint8_t)(int(ab.y * 255.0) & 0xFF);
    }
