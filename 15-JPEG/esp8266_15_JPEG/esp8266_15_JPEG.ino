...
void loop() {
  if ((DATA_IN_CNT >= DATA_IN_SIZE) && (millis() - lastComputeTime > COMPUTE_PERIOD_MS)) {
    ...
    JFIF mJFIF(DATA_IN, DATA_IN_SIZE);
    const std::vector< std::vector<int> >* mJpeg = mJFIF.jpeg();
    int mIiSize = mJpeg->size();
    int mIjSize = (*mJpeg)[0].size();

    for (int i = 0; i < DATA_OUT_SIZE; i++) {
      int iv = (*mJpeg)[(i / mIjSize) % mIiSize][i % mIjSize];
