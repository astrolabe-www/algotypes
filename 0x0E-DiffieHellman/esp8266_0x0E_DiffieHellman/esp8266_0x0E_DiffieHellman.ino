...
uint32_t COMPUTE_PERIOD_MS = 400;
...
const int DATA_OUT_SIZE = DATA_IN_SIZE / 2;
...
void loop() {
  if ((DATA_IN_CNT >= DATA_IN_SIZE) && (millis() - lastComputeTime > COMPUTE_PERIOD_MS)) {
    ...
    for (int i = 0; i < DATA_OUT_SIZE; i++) {
      int aliceKeys[] = { (DATA_IN[(2 * i + 0) % DATA_IN_SIZE] ^ 0x00) & 0xFF,
                          max(1, (DATA_IN[(2 * i + 1) % DATA_IN_SIZE] ^ 0x00) & 0xFF)
                        };
      int bbobbKeys[] = { (DATA_IN[(2 * i + 0) % DATA_IN_SIZE] ^ 0xFF) & 0xFF,
                          max(1, (DATA_IN[(2 * i + 1) % DATA_IN_SIZE] ^ 0xFF) & 0xFF)
                        };

      DiffieHellman alice = DiffieHellman(aliceKeys[0], aliceKeys[1]);
      DiffieHellman bbobb = DiffieHellman(bbobbKeys[0], bbobbKeys[1]);

      int fullKey = alice.generateFullKey(bbobb.generatePartialKey(alice.publicKey), bbobb.publicKey);

      DATA_OUT[i] = (uint8_t)(fullKey & 0xFF);
    }
