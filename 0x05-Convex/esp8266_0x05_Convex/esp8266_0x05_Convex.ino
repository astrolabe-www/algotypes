...
const uint8_t mCHANNEL = 5;
const String mSSID = "0x05-Convex";
...
uint32_t COMPUTE_PERIOD_MS = 700;
...
void loop() {
  if ((DATA_IN_CNT >= DATA_IN_SIZE) && (millis() - lastComputeTime > COMPUTE_PERIOD_MS)) {
    ...
    int polygonsPerRound = 8;
    int oI = 0;

    while (oI < DATA_OUT_SIZE) {
      for (int p = 0; p < polygonsPerRound; p++) {
        int inputsPerPolygon = DATA_IN_SIZE / polygonsPerRound;

        Polygon mPolygon = Polygon(DATA_IN + (p * inputsPerPolygon), inputsPerPolygon);
        std::vector<Point> mHull = mPolygon.convexHull();

        for (int h = 0; h < mHull.size(); h++) {
          DATA_OUT[oI % DATA_OUT_SIZE] = (uint8_t)(mHull[h].x & 0xFF);
          oI += 1;
          DATA_OUT[oI % DATA_OUT_SIZE] = (uint8_t)(mHull[h].y & 0xFF);
          oI += 1;
        }
      }
    }
