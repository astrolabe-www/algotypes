...
const uint8_t mCHANNEL = 9;
const String mSSID = "0x09-SHA3";
...
void loop() {
  if ((DATA_IN_CNT >= DATA_IN_SIZE) && (millis() - lastComputeTime > COMPUTE_PERIOD_MS)) {
    ...
    Keccak mKeccak(576, 1024);
    std::vector<uint8_t> sha3 = mKeccak.SHA3(DATA_IN, DATA_IN_SIZE);

    for (int i = 0; i < DATA_OUT_SIZE; i++) {
      DATA_OUT[i] = (uint8_t)(sha3[i % sha3.size()] & 0xFF);
    }
    DATA_IN_CNT = 0;
