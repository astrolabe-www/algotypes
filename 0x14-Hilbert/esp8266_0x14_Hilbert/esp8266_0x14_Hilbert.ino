...
const String mSSID = "0x14-Hilbert";
...
void loop() {
  if ((DATA_IN_CNT >= DATA_IN_SIZE) && (millis() - lastComputeTime > COMPUTE_PERIOD_MS)) {
    ...
    for (int i = 0; i < DATA_OUT_SIZE / 2; i++) {
      int input = ((DATA_IN[i % DATA_IN_SIZE] << 8) & 0xFF00) | (DATA_IN[(i + 1) % DATA_IN_SIZE] & 0xFF);
      Vec2 xy = Hilbert::d2xy(input);
      DATA_OUT[2 * i + 0] = (uint8_t)(int(xy.x) & 0xFF);
      DATA_OUT[2 * i + 1] = (uint8_t)(int(xy.y) & 0xFF);
    }
    DATA_IN_CNT = 0;
