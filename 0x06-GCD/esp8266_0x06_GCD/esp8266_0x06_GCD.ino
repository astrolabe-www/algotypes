...
const uint8_t mCHANNEL = 6;
const String mSSID = "0x06-GCD";
...
void loop() {
  if ((DATA_IN_CNT >= DATA_IN_SIZE) && (millis() - lastComputeTime > COMPUTE_PERIOD_MS)) {
    ...
    for (int i = 0; i < DATA_OUT_SIZE; i++) {
      int a = min(DATA_IN[i % DATA_IN_SIZE], DATA_IN[(i + 1) % DATA_IN_SIZE]);
      int b = max(DATA_IN[i % DATA_IN_SIZE], DATA_IN[(i + 1) % DATA_IN_SIZE]);
      DATA_OUT[i] = (uint8_t)(Euclid::gcd(a, b) & 0xFF);
    }
    DATA_IN_CNT = 0;
