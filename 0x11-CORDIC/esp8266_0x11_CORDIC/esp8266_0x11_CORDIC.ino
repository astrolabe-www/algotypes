...
const uint8_t mCHANNEL = 4; // 17 % 13
const String mSSID = "0x11-CORDIC";
...
uint32_t COMPUTE_PERIOD_MS = 100;
...
void loop() {
  if ((DATA_IN_CNT >= DATA_IN_SIZE) && (millis() - lastComputeTime > COMPUTE_PERIOD_MS)) {
    ...
    for (int i = 0; i < DATA_OUT_SIZE / 2; i++) {
      float A = TWO_PI * DATA_IN[i % DATA_IN_SIZE] / 255.0f;
      Vec2 c = CORDIC::cossin(A);

      int outCos = (int)(127.5f * (c.x + 1.0));
      int outSin = (int)(127.5f * (c.y + 1.0));

      DATA_OUT[2 * i + 0] = (uint8_t)(outCos & 0xFF);
      DATA_OUT[2 * i + 1] = (uint8_t)(outSin & 0xFF);
    }
