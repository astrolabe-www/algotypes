...
const String mSSID = "0x0C-Inversion";
...
uint32_t COMPUTE_PERIOD_MS = 800;
...
void loop() {
  if ((DATA_IN_CNT >= DATA_IN_SIZE) && (millis() - lastComputeTime > COMPUTE_PERIOD_MS)) {
    ...
    LUPMatrix mLUPMatrix(DATA_IN, DATA_IN_SIZE);
    const std::vector< std::vector<float> >* mI = mLUPMatrix.inverted();
    int mIiSize = mI->size();
    int mIjSize = (*mI)[0].size();

    for (int i = 0; i < DATA_OUT_SIZE; i++) {
      float fv = (*mI)[(i / mIjSize) % mIiSize][i % mIjSize];
      int iv = (int)(*((int*)(&fv)));
