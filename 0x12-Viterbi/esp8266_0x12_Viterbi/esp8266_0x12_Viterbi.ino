...
const uint8_t mCHANNEL = 5; // 18 % 13
const uint8_t mMAC = 0x52;
const String mSSID = "0x12-Viterbi";
...
const int DATA_OUT_SIZE = DATA_IN_SIZE;
...
void loop() {
  if ((DATA_IN_CNT >= DATA_IN_SIZE) && (millis() - lastComputeTime > COMPUTE_PERIOD_MS)) {
    ...
    FSM mFSM = FSM(DATA_IN, DATA_IN_SIZE);
    int oI = 0;
    int s = 0;
    while (oI < DATA_OUT_SIZE) {
      int inputOffset = (s++) * FSM::NUMBER_STAGES;
      if (inputOffset + FSM::NUMBER_STAGES > DATA_IN_SIZE) inputOffset = inputOffset % DATA_IN_SIZE;
      int* out = mFSM.calculate(DATA_IN + inputOffset);

      for (int i = 0; i < FSM::NUMBER_STAGES / 2; i++) {
