...
const uint8_t mCHANNEL = 10;
const String mSSID = "0x0A-Perlin";
...
void loop() {
  if ((DATA_IN_CNT >= DATA_IN_SIZE) && (millis() - lastComputeTime > COMPUTE_PERIOD_MS)) {
    ...
    Perlin mPerlin(DATA_IN, DATA_IN_SIZE);

    for (int i = 0; i < DATA_OUT_SIZE; i++) {
      DATA_OUT[i] = (uint8_t)((int)(255.0 * mPerlin.noise((float)(i) / 32.0)) & 0xFF);
    }
    DATA_IN_CNT = 0;
